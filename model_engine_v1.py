#!/usr/bin/env python3
"""Backtest-leading explainable probability engine (v1 production core).

This is intentionally frozen from the pre-xG model that produced the stronger
rolling Top-10 hit rate in the historical comparison. The xG-aware model remains
in model_engine.py as a diagnostic/helper model, not the primary selection engine.
"""
...
def _weighted_mean(values:Sequence[Optional[float]],half_life:float=DEFAULT_HALF_LIFE_MATCHES)->Tuple[Optional[float],int]:
    total=weights=0.0;count=0;decay=math.exp(math.log(.5)/max(.1,half_life))
...
def _ratio(value:float,baseline:float)->float:
    return 1.0 if baseline<=1e-9 else _clip(value/baseline,.35,2.50)
...
def predict_match(history:Sequence[Dict[str,Any]],home_team:str,away_team:str,*,recent_matches:int=DEFAULT_RECENT_MATCHES)->Prediction:
    ...
    p_corners=poisson_prob_over(8,lam_corners)
    home_sample=max(h_n,hc_n,len(h_all));away_sample=max(a_n,ac_n,len(a_all))
    quality=_clip(min(home_sample,away_sample)/15.0,.20,1.0)
    return Prediction(p_over25,p_btts,p_corners,lam_h,lam_a,lam_corners,home_sample,away_sample,quality)

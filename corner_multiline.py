...
module only evaluates that same lambda at the Turkish/international line that is
actually offered; it does not change the underlying V1 model parameters.
"""
...
def corner_line_from_market(market: Any) -> Optional[float]:
    ...
    if not text.startswith(prefix):
        return None
    raw = text[len(prefix):].replace("_", ".")
    return normalize_corner_line(raw)
...
def corner_probability(lambda_total_corners: Any, line: Any) -> float:
    ...
    lam = float(lambda_total_corners)
    # For a half-line x.5, Over wins at integer total >= floor(x.5)+1.
    return float(poisson_prob_over(int(math.floor(x)), lam))

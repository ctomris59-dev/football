#!/usr/bin/env python3
"""Ordered live refresh pipeline with serialization and duplicate-run cooldown.

Provider calls are freshness/coverage-gated. Optional sources fail soft; core
mapping/readiness fails closed. A PostgreSQL advisory lock prevents concurrent runs,
and a persistent cooldown prevents duplicate Thursday cron jobs from rerunning the
full pipeline sequentially.
"""
...
REFRESH_TRIGGER_NAME = os.getenv("LIVE_REFRESH_TRIGGER_NAME", "scheduled").strip() or "scheduled"


def envb(key: str, default: str = "true") -> bool:
    return os.getenv(key, default).lower() in {"1", "true", "yes"}


RUN_FD2324 = envb("LIVE_REFRESH_FD2324")
RUN_FOOTBALL_DATA = envb("LIVE_REFRESH_FOOTBALL_DATA")
RUN_FD_ASIAN = envb("LIVE_REFRESH_FD_ASIAN")
RUN_ESPN = envb("LIVE_REFRESH_ESPN")
RUN_ESPN_CONTEXT = envb("LIVE_REFRESH_ESPN_CONTEXT")
RUN_ESPN_TOTAL_ODDS = envb("LIVE_REFRESH_ESPN_TOTAL_ODDS")
RUN_ESPN_TEAM_SCHEDULE = envb("LIVE_REFRESH_ESPN_TEAM_SCHEDULE")
RUN_UNDERSTAT = envb("LIVE_REFRESH_UNDERSTAT")
RUN_ODDSPAPI = envb("LIVE_REFRESH_ODDSPAPI")
RUN_FOTMOB_AVAILABILITY = envb("LIVE_REFRESH_FOTMOB_AVAILABILITY")
RUN_FOTMOB_LINEUPS = envb("LIVE_REFRESH_FOTMOB_LINEUPS")
RUN_BBS = envb("LIVE_REFRESH_BBS", "false")
RUN_BBS_LINEUPS = envb("LIVE_REFRESH_BBS_LINEUPS", "false")
RUN_SOFASCORE = envb("LIVE_REFRESH_SOFASCORE", "false")
RUN_ADVANCED = envb("LIVE_REFRESH_ADVANCED")
RUN_FOUR_LAYER = envb("LIVE_REFRESH_FOUR_LAYER")
RUN_PREMATCH = envb("LIVE_REFRESH_PREMATCH")
RUN_ODDS_MOVEMENT = envb("LIVE_REFRESH_ODDS_MOVEMENT")
RUN_AVAILABILITY_ENRICH = envb("LIVE_REFRESH_AVAILABILITY_ENRICH")
RUN_READINESS = envb("LIVE_REFRESH_READINESS")
RUN_PREDICTIONS = envb("LIVE_REFRESH_PREDICTIONS")
RUN_TURKEY_ODDS = envb("LIVE_REFRESH_TURKEY_ODDS", "true")
RUN_TURKEY_LISTS = envb("LIVE_REFRESH_TURKEY_LISTS", "true")
...
FOTMOB_REFRESH_HOURS = float(os.getenv("FOTMOB_REFRESH_HOURS", "12"))
FOTMOB_LINEUP_REFRESH_HOURS = float(os.getenv("FOTMOB_LINEUP_REFRESH_HOURS", "0.5"))
FOTMOB_MIN_ACTIVE_COVERAGE = float(os.getenv("FOTMOB_MIN_ACTIVE_COVERAGE", "0.85"))
...
def recent_rows(table: str, hours: float) -> bool:
    if table not in {"asian_market_prices"} or not DATABASE_URL:
        return False
    try:
        with psycopg.connect(DATABASE_URL) as conn:
            return bool(conn.execute(
                f"SELECT 1 FROM {table} WHERE fetched_at>=NOW()-(%s||' hours')::interval LIMIT 1",
                (hours,),
            ).fetchone())
    except Exception:
        return False
...
def fotmob_availability_fresh() -> tuple[bool, Dict[str, Any]]:
    if not DATABASE_URL or FOTMOB_REFRESH_HOURS <= 0:
        return False, {"reason": "disabled freshness"}
    try:
        with psycopg.connect(DATABASE_URL) as conn:
            row = conn.execute(
                """WITH active AS (
                       SELECT event_id FROM espn_upcoming
                       WHERE is_current=TRUE AND match_date>=NOW()-INTERVAL '2 hours'
                         AND match_date<=NOW()+INTERVAL '8 days'
                     ), covered AS (
                       SELECT DISTINCT f.espn_event_id
                       FROM fotmob_fixture_availability_snapshots f JOIN active a ON a.event_id=f.espn_event_id
                       WHERE f.fetched_at>=NOW()-(%s||' hours')::interval
                     )
                     SELECT (SELECT COUNT(*) FROM active),(SELECT COUNT(*) FROM covered)""",
                (FOTMOB_REFRESH_HOURS,),
            ).fetchone()
        total, covered = int(row[0] or 0), int(row[1] or 0)
        rate = covered / total if total else 0.0
        meta = {"active": total, "fresh_covered": covered, "rate": round(rate, 4), "max_age_hours": FOTMOB_REFRESH_HOURS, "min_rate": FOTMOB_MIN_ACTIVE_COVERAGE}
        return bool(total > 0 and rate >= FOTMOB_MIN_ACTIVE_COVERAGE), meta
    except Exception as exc:
        return False, {"error": str(exc)[:300]}


def skip(steps, name, reason):
...
def _run() -> Dict[str, Any]:
    started = utcnow()
    summary = {"started_at": started.isoformat(), "steps": {}}
    steps = summary["steps"]
    ...
    if RUN_ESPN_TOTAL_ODDS:
        from espn_total_odds_bridge import run_import as fn
        run_step("espn_total_odds", lambda: fn(DATABASE_URL), steps, optional=True)
    ...
    if RUN_FD_ASIAN:
        from football_data_asian_bridge_v2 import run_import as fn
        run_step("football_data_asian", lambda: fn(DATABASE_URL), steps, optional=True)
    if RUN_FOTMOB_AVAILABILITY:
        fresh, meta = fotmob_availability_fresh()
        if fresh:
            skip(steps, "fotmob_availability", f"active coverage fresh {meta}")
        else:
            log.info("FOTMOB_AVAILABILITY_REFRESH_REQUIRED %s", json.dumps(meta, separators=(",", ":")))
    ...
    if RUN_FOTMOB_LINEUPS:
        if recent_success("fotmob_lineup_runs", FOTMOB_LINEUP_REFRESH_HOURS):
            skip(steps, "fotmob_lineups", f"fresh<{FOTMOB_LINEUP_REFRESH_HOURS}h")
        else:
            from fotmob_lineups_importer import run_import as fn
            run_step("fotmob_lineups", lambda: fn(DATABASE_URL), steps, optional=True)
    if RUN_BBS:
    ...
    if RUN_FOUR_LAYER:
        from player_context_orchestrator import run as fn
        run_step("player_context_db_v3", lambda: fn(DATABASE_URL), steps)
        from player_context_enrichment_bridge import run_bridge as fn
        run_step("player_context_bridge", lambda: fn(DATABASE_URL), steps)
        from pressure_features_builder import build as fn
        run_step("pressure_features", lambda: fn(DATABASE_URL), steps, optional=True)
    if RUN_PREMATCH:
        from prematch_context_builder_fixed import run_build as fn
        run_step("prematch_context", lambda: fn(DATABASE_URL), steps)
    if RUN_FOUR_LAYER:
        from asian_market_features_builder import run_build as fn
        run_step("asian_market_features", lambda: fn(DATABASE_URL), steps, optional=True)
        from advanced_context_v4_builder import run_build as fn
        run_step("advanced_context_v4", lambda: fn(DATABASE_URL), steps, optional=True)
        from four_layer_coverage_audit import run_audit as fn
        run_step("four_layer_coverage", lambda: fn(DATABASE_URL), steps)
        from v1_v5_policy_backtest import ensure_validation as fn
        run_step("v1_v5_policy_validation", lambda: fn(DATABASE_URL), steps)
    ...
    if RUN_AVAILABILITY_ENRICH:
        from availability_enricher_v4 import run_enrich as fn
        run_step("availability_enrich", lambda: fn(DATABASE_URL), steps)
    if RUN_READINESS:
        from data_readiness_audit_v5 import run_audit as fn
        run_step("data_readiness", lambda: fn(DATABASE_URL), steps)
    if RUN_PREDICTIONS:
        from production_predictor_v5 import run_predictions as fn
        run_step("production_predictions", lambda: fn(DATABASE_URL), steps)
    if RUN_TURKEY_ODDS:
        from turkey_iddaa_odds_collector import run_import as fn
        run_step("turkey_iddaa_odds", lambda: fn(DATABASE_URL), steps, optional=True)
    if RUN_TURKEY_LISTS:
        from turkey_value_workflow import build_lists as fn
        run_step("turkey_dual_lists", lambda: fn(DATABASE_URL), steps)
...
def main() -> Dict[str, Any]:
    ...
    try:
        ...
        if not got:
            return {"status": "skipped_duplicate_refresh", "reason": "postgres_advisory_lock_busy", "lock_key": REFRESH_LOCK_KEY, "at": utcnow().isoformat()}

        guard_id, skipped = guard_begin(lock, REFRESH_MIN_INTERVAL_MINUTES, REFRESH_FORCE, REFRESH_TRIGGER_NAME)
        if skipped:
            log.info("LIVE_REFRESH_COOLDOWN_SKIP %s", json.dumps(skipped, default=str, separators=(",", ":")))
            return skipped
        try:
            result = _run()
            guard_finish(lock, guard_id, "success", json.dumps({"finished_at": result.get("finished_at")}, default=str))
            return result
        except Exception as exc:

...
def fotmob_availability_fresh()->tuple[bool,Dict[str,Any]]:
 """Require fresh rows for ACTIVE fixtures; a zero-row 'successful run' is not fresh."""
 if not DATABASE_URL or FOTMOB_REFRESH_HOURS<=0:return False,{"reason":"disabled freshness"}
 try:
  with psycopg.connect(DATABASE_URL) as conn:
   row=conn.execute("""WITH active AS (
       SELECT event_id FROM espn_upcoming WHERE is_current=TRUE AND match_date>=NOW()-INTERVAL '2 hours' AND match_date<=NOW()+INTERVAL '8 days'
     ), covered AS (
       SELECT DISTINCT f.espn_event_id FROM fotmob_fixture_availability_snapshots f JOIN active a ON a.event_id=f.espn_event_id
       WHERE f.fetched_at>=NOW()-(%s||' hours')::interval
     ) SELECT (SELECT COUNT(*) FROM active),(SELECT COUNT(*) FROM covered)""",(FOTMOB_REFRESH_HOURS,)).fetchone()
  total,covered=int(row[0] or 0),int(row[1] or 0);rate=covered/total if total else 0.0

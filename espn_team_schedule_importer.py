#!/usr/bin/env python3
"""Collect broader team schedules for clubs in upcoming Big Five fixtures.

The domestic ESPN team schedule alone is not sufficient for fatigue/rest context.
For every upcoming Big Five club we query its domestic competition plus UEFA
Champions League, Europa League and Conference League schedule pages. Returned
events are merged by team/event id and preserved with competition metadata.
"""
...
class Importer:
    ...
    def teams(self) -> List[Tuple[str, str, str]]:
        rows = self.conn.execute(
            """SELECT DISTINCT league_slug,home_team_id,home_team
                 FROM espn_upcoming
                WHERE is_current=TRUE AND home_team_id IS NOT NULL
               UNION
               SELECT DISTINCT league_slug,away_team_id,away_team
                 FROM espn_upcoming
                WHERE is_current=TRUE AND away_team_id IS NOT NULL"""
        ).fetchall()
    ...
    def store_event(
        ...
        *,
        ...
        comp_name = str(league_obj.get("name") or league_obj.get("abbreviation") or source_slug) or source_slug
        comp_slug = str(league_obj.get("slug") or source_slug) or source_slug
        self.conn.execute(
            """INSERT INTO espn_team_schedule_events(
                    league_slug,team_id,team_name,event_id,match_date,
                    competition_name,competition_slug,opponent_name,home_away,
                ...
                ON CONFLICT(team_id,event_id) DO UPDATE SET
                    ...
                    raw=EXCLUDED.raw,
                    updated_at=NOW()""",
            (league, team_id, team_name, eid, dt, comp_name, comp_slug, opp_name, homeaway, status, completed, Jsonb(e)),
        )
        return True

    def run(self) -> Dict[str, Any]:
        rid = self.conn.execute("INSERT INTO espn_team_schedule_runs(status) VALUES('running') RETURNING id").fetchone()[0]
        teams = events = requests_done = 0
        try:
            for domestic_slug, tid, tname in self.teams():
                query_slugs = (domestic_slug,) + tuple(x for x in UEFA_SCHEDULE_SLUGS if x != domestic_slug)
                for source_slug in query_slugs:
                    payload = self.get(f"{BASE}/{source_slug}/teams/{tid}/schedule")
                    requests_done += 1
                    for e in as_events(payload):
                        events += int(self.store_event(domestic_slug, tid, tname, e, source_slug=source_slug))
                teams += 1
            self.conn.execute(
                "UPDATE espn_team_schedule_runs SET finished_at=NOW(),status='success',teams=%s,events=%s,message=%s WHERE id=%s",
                (teams, events, f"ok requests={requests_done}", rid),
            )
            result = {"status": "success", "teams": teams, "events": events, "requests": requests_done, "uefa_slugs": list(UEFA_SCHEDULE_SLUGS)}
            log.info("ESPN_TEAM_SCHEDULE_RESULT %s", json.dumps(result, separators=(",", ":")))
            return result
        except Exception as exc:
            self.conn.execute(
                "UPDATE espn_team_schedule_runs SET finished_at=NOW(),status='failed',teams=%s,events=%s,message=%s WHERE id=%s",
                (teams, events, str(exc)[:1000], rid),
            )
            raise


def run_import(database_url: Optional[str] = None) -> Dict[str, Any]:
    importer = Importer(database_url)
    try:
        return importer.run()
    finally:
        importer.close()

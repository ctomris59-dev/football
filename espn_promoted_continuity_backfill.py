...
# ESPN and Football-Data use different labels for the same German clubs.
# Keep this narrowly scoped to aliases proven by the live promoted-history audit.
TARGET_ALIASES={
 "hamburg sv":"hamburg",
...
def target_canon(v:Any)->str:
    c=v2.canon(v)
    return TARGET_ALIASES.get(c,c)

def stored_team_name(name:str)->str:
    c=v2.canon(name)
    return TARGET_ALIASES.get(c,name)
...
def current_promoted(conn)->Dict[str,Set[str]]:
    ...
    try:
        for team,parent in conn.execute("SELECT team_name,parent_league_name FROM promotion_priors WHERE target_season='2627'").fetchall():
            if str(parent) in out:out[str(parent)].add(target_canon(team))
    except Exception:pass
    for league in out:
        try:
            previous={target_canon(x[0]) for x in conn.execute("""SELECT home_team FROM football_data_matches WHERE season_code='2526' AND league_name=%s
              UNION SELECT away_team FROM football_data_matches WHERE season_code='2526' AND league_name=%s""",(league,league)).fetchall()}
            current={target_canon(x[0]) for x in conn.execute("""SELECT home_team FROM espn_upcoming WHERE is_current=TRUE AND league_name=%s
...
def backtest_promoted(conn)->Dict[str,Set[str]]:
    ...
    for league in out:
        try:
            prev={target_canon(x[0]) for x in conn.execute("""SELECT home_team FROM football_data_matches WHERE season_code='2425' AND league_name=%s
              UNION SELECT away_team FROM football_data_matches WHERE season_code='2425' AND league_name=%s""",(league,league)).fetchall()}
            test={target_canon(x[0]) for x in conn.execute("""SELECT home_team FROM football_data_matches WHERE season_code='2526' AND league_name=%s
              UNION SELECT away_team FROM football_data_matches WHERE season_code='2526' AND league_name=%s""",(league,league)).fetchall()}
...
def target_state(conn,season:int,league_slug:str,targets:Set[str])->Dict[str,int|float]:
    ...
    vals=[r for r in rows if target_canon(r[0]) in targets or target_canon(r[1]) in targets]
            ...
            for parent,(slug,lname) in SECOND_TIERS.items():
                ...
                for start,end in month_windows(start_s,end_s):
                    ...
                    for event in events:
                        ...
                        if target_canon(hn) not in targets and target_canon(an) not in targets:continue
                ...
                pending=[r for r in pending if any(target_canon(x) in targets for x in i.conn.execute("SELECT home_team,away_team FROM espn_historical_events WHERE event_id=%s",(r[0],)).fetchone())][:MAX_SUMMARIES]
                ...
                for eid,hid,aid in pending:
                    try:
                        ...
                        for tid,g in groups.items():
                            team_label=stored_team_name(str(g["team_name"]))
                            for p in g["players"].values():
                                i.conn.execute("""INSERT INTO espn_historical_lineup_players(event_id,team_id,team_name,player_id,player_name,starter,source_path,raw)
                                  VALUES(%s,%s,%s,%s,%s,TRUE,%s,%s) ON CONFLICT(event_id,team_id,player_id) DO UPDATE SET team_name=EXCLUDED.team_name,player_name=EXCLUDED.player_name,starter=TRUE,source_path=EXCLUDED.source_path,raw=EXCLUDED.raw,fetched_at=NOW()""",
                                  (str(eid),tid,team_label,p["player_id"],p["player_name"],p["source_path"],Jsonb(p["raw"])));starter_rows+=1

...
RUN_LINEUP_V2_AUDIT_ONCE = os.getenv(
    "RUN_LINEUP_V2_AUDIT_ONCE", "false"
).strip().lower() in {"1", "true", "yes"}
...
def _run_over25() -> None:
    ...
    try:
        ...
        _over25_state["status"] = "failed"; _over25_state["error"] = str(exc)[:2000]
...
def _run_incremental() -> None:
    ...
    try:
        ...
        result = run(DATABASE_URL)
        _incremental_state["result"] = result; _incremental_state["status"] = "success"
        log.info("INCREMENTAL_FEATURE_CONTROLLER_COMPLETED %s", result)
    except Exception as exc:
        _incremental_state["status"] = "failed"; _incremental_state["error"] = str(exc)[:2000]
...
def _run_lineup_v2() -> None:
    ...
    try:
        ...
        result = run(DATABASE_URL)
        _lineup_v2_state["result"] = result; _lineup_v2_state["status"] = "success"
        log.info("LINEUP_V2_CONTROLLER_COMPLETED %s", result)
    except Exception as exc:
        _lineup_v2_state["status"] = "failed"; _lineup_v2_state["error"] = str(exc)[:2000]
        log.exception("LINEUP_V2_CONTROLLER_FAILED")
    finally:
...
if DATABASE_URL and RUN_LINEUP_V2_AUDIT_ONCE:
    _ensure_lineup_v2_started()

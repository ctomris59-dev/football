...
# Six independent bookmaker requests are still very small relative to the rest of
# the refresh, but materially improve the chance that at least two books expose a
# valid paired market for the exact same fixture/line.
...
def main(database_url: str = DATABASE_URL, *, now: Optional[datetime] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        # Refresh Understat before the model run. Historical seasons are cached for
        # years, while the live season has its own short freshness window.
        try:
        ...
        # Raise the independent bookmaker request pool before the reference module
        # imports/runs the V4 collector. This changes coverage, not model thresholds.
        try:
        ...
        if core.get("ready"):
            ...
            payload = {
                ...
                "sources": {
                    "model": "V4 six-layer: V1 + opponent-adjusted xG + opponent-adjusted goals + XI + Dixon-Coles",
                    "international": "fresh multi-book same-market no-vig consensus",

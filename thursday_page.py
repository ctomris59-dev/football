...
Primary list: the week's reliability-ranked V1 predictions among playable Turkish
markets. Strict >=70% entries are labelled High Confidence inside the cards. Value is
an optional second list and may legitimately be empty.
"""
...
<main class="wrap">
  <header class="hero">
    <div><div class="eyebrow">Football Decision Engine</div><h1>Perşembe Bahis Listesi</h1><p>Türkiye bülteni yayınlanır → V1 tüm oynanabilir marketleri sıralar → haftanın güvenilir listesi oluşur. Value varsa ayrıca gösterilir.</p></div>
  ...
  <section class="status">
    ...
    <div class="flow">
      ...
      <div class="step"><b>2. Türkiye oranı</b><span>Yalnız gerçekten oynanabilir taraflar</span></div>
      <div class="step"><b>3. Güven sırası</b><span>Her maçtan en güçlü seçim</span></div>
      <div class="step"><b>4. Value</b><span>Varsa ayrı ve daha sıkı filtre</span></div>
    </div>
  </section>

  <div class="grid">
    <section class="section"><div class="sectionHead"><h2>🛡️ Haftanın En Güvenilirleri</h2><p>Türkiye'de oynanabilir marketlerden V1 sıralamasında en güçlü seçimler. Gerçekten ≥%70 olanlar ayrıca “Yüksek Güven” olarak işaretlenir.</p></div><div class="list" id="highList"><div class="empty">Liste henüz kesinleşmedi.</div></div></section>
    <section class="section"><div class="sectionHead"><h2>💰 Yüksek Güven + Value</h2><p>Dünya piyasası doğrulaması ve Türkiye fiyat avantajını birlikte geçen daha sıkı alt küme. Her hafta çıkması şart değildir.</p></div><div class="list" id="valueList"><div class="empty">Liste henüz kesinleşmedi.</div></div></section>
  </div>
  <div class="footer">Gösterilen yüzde V1 model tahminidir; garanti veya kusursuz kalibre edilmiş gerçek olasılık değildir. “Yüksek Güven” etiketi yalnız modelin seçilen tarafa verdiği tahmin ≥%70 ise kullanılır. Value ayrı bir ekonomik filtredir.</div>
</main>
<script>
...
function pickCard(p,isValue){const card=document.createElement('article');card.className='pick';const t=document.createElement('div');t.className='teams';t.textContent=`${p.home||''} – ${p.away||''}`;const m=document.createElement('div');m.className='market';m.textContent=p.selection||p.market||'—';const tier=document.createElement('div');tier.className='tier'+(p.strict_high_confidence?' strict':'');tier.textContent=p.strict_high_confidence?'YÜKSEK GÜVEN':(p.confidence_tier||'HAFTANIN EN GÜVENİLİRLERİ');const ms=document.createElement('div');ms.className='metrics';ms.append(metric('V1 model tahmini',pct(p.model_probability_estimate??p.confidence),true));ms.append(metric('Dünya fair',pct(p.international_fair_probability||p.international_probability||p.market_reference_probability)));ms.append(metric('Türkiye oranı',odd(p.tr_price||p.tr_opening_price)));if(isValue){ms.append(metric('Model EV',pct(p.model_ev_vs_tr||p.ev),true));}card.append(t,m,tier,ms);return card}
function renderList(id,items,isValue){const el=$(id);clear(el);if(!items||!items.length){const d=document.createElement('div');d.className='empty';d.textContent=isValue?'Bu hafta uygun value bahis yok.':'Bülten yeterli olduğunda haftanın sıralı tahminleri burada görünecek.';el.appendChild(d);return}items.forEach(p=>el.appendChild(pickCard(p,isValue)))}
async function loadData(){try{const r=await fetch('/thursday-list',{cache:'no-store'});const d=await r.json();$('updated').textContent='Son kontrol: '+new Date().toLocaleString('tr-TR');const finalized=d.status==='finalized';$('badge').className='badge '+(finalized?'final':'pending');$('badgeText').textContent=finalized?'LİSTE DONDURULDU':'BÜLTEN BEKLENİYOR';$('statusTitle').textContent=finalized?'Bu haftanın tahmin listesi hazır':'Haftalık karar henüz hazır değil';$('statusText').textContent=finalized?`Liste ${d.finalized_at?new Date(d.finalized_at).toLocaleString('tr-TR'):''} tarihinde kilitlendi.`:'Türkiye bülteni yeterli kapsama ulaşınca güvenilir tahmin listesi otomatik oluşacak; value için beklemek zorunda değil.';if(finalized){renderList('highList',d.high_confidence,false);renderList('valueList',d.high_confidence_value,true)}}catch(e){$('statusTitle').textContent='Bağlantı hatası';$('statusText').textContent='Sayfayı yenileyerek tekrar dene.'}}

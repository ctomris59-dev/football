...
def athlete_from_entry(obj:Any)->Optional[Dict[str,Any]]:
    ...
    if player_name(obj) and any(k in obj for k in ("position","jersey","uid","guid","starter","subbedIn","subbedOut","athleteId")):
        return dict(obj)
    return None
...
def team_identity(team:Any)->Tuple[Optional[str],Optional[str]]:
    ...
    name=team.get("displayName") or team.get("shortDisplayName") or team.get("name") or team.get("abbreviation")
...
def grouped_team_players(raw:Any)->List[Tuple[Optional[str],Optional[str],Optional[str],List[Dict[str,Any]],str]]:
    ...
    def walk(x:Any,path:str="root",inherited_team:Any=None,inherited_side:Optional[str]=None):
        if isinstance(x,dict):
            team=x.get("team") or inherited_team;tid,tname=team_identity(team);side=side_hint(x) or inherited_side
            candidates=[]
            for k in keys:
                v=x.get(k)
            ...
            for k,v in x.items():walk(v,f"{path}.{k}",team,side)
        elif isinstance(x,list):
            for i,v in enumerate(x):walk(v,f"{path}[{i}]",inherited_team,inherited_side)
    walk(raw);return groups

def schema_signature(raw:Any,limit:int=30)->List[str]:
    """Return paths + keys/list sizes only; never values or player names."""
    sig=[]
    def walk(x:Any,path:str="root",depth:int=0):
        if len(sig)>=limit or depth>5:return
        if isinstance(x,dict):
            keys=sorted(str(k) for k in x.keys())
            interesting=[k for k in keys if any(n in k.lower() for n in ("roster","line","athlet","player","team","home","away"))]
            if interesting:sig.append(f"{path}:dict[{','.join(interesting[:12])}]")
            for k,v in x.items():
                if isinstance(v,(dict,list)):walk(v,f"{path}.{k}",depth+1)
        elif isinstance(x,list):
            sig.append(f"{path}:list[{len(x)}]")
            if x:walk(x[0],f"{path}[0]",depth+1)
    walk(raw);return sig[:limit]
...
def strength_rows(players:List[Dict[str,Any]])->Tuple[List[Dict[str,Any]],int]:
    ...
    for i,p in enumerate(players):
        ...
        flag=flags[i];starts=1.0 if flag is True else 0.0;minutes=90.0 if flag is True else 15.0
        pos=p.get("position") or ((p.get("_entry") or {}).get("position") if isinstance(p.get("_entry"),dict) else None)
        out.append({"player_id":str(p.get("id") or p.get("uid") or p.get("guid") or p.get("athleteId") or canon(name)),"player_name":name,
                    "games":1.0,"starts":starts,"minutes":minutes,"goals":0.0,"xg":0.0,"assists":0.0,"xa":0.0,"xgchain":0.0,"xgbuildup":0.0,
                    "raw":{"source":"espn-prematch-roster","starter":flag,"position":pos}})
    return out,explicit
...
def run_bridge(database_url:Optional[str]=None)->Dict[str,Any]:
    ...
    with psycopg.connect(db,autocommit=True) as conn:
        ...
        rows=conn.execute("""SELECT DISTINCT ON(p.event_id) p.event_id,p.raw,COALESCE(u.home_team_id,''),u.home_team,COALESCE(u.away_team_id,''),u.away_team
        ...
        for ix,(eid,raw,hid,hname,aid,aname) in enumerate(rows):
            ...
            groups_seen+=len(groups);targets=[("home",str(hid or ""),str(hname)),("away",str(aid or ""),str(aname))]
            for tid,tname,side,players,path in groups:
                ...
                for target_side,target_id,target_name in targets:
                    ...
                    elif side and side==target_side:score=.75
                    if score>best_score:best_score=score;best=(target_id,target_name)
                if not best:continue
                _target_id,target_name=best;parsed,explicit=strength_rows(players)
        ...
        sql="""INSERT INTO player_team_context_snapshots(team_name,snapshot_hour,current_season,previous_season,expected_xi_strength,top11_strength,injury_impact,goalkeeper_injured,retained_minutes_share,starter_continuity,player_coverage,key_absences,source_meta)
               ...
               injury_impact=0.0,goalkeeper_injured=FALSE,retained_minutes_share=NULL,starter_continuity=NULL,player_coverage=EXCLUDED.player_coverage,key_absences='[]'::jsonb,source_meta=EXCLUDED.source_meta"""
        params=[]
        for team,rec in teams.items():
            ...
            ranked=sorted(players,key=lambda p:(float(p.get("starts") or 0),float(p.get("minutes") or 0),scores.get(p["player_id"],.5)),reverse=True);top=ranked[:11];topvals=[scores.get(p["player_id"],.5) for p in top]
            expected=sum(topvals)/len(topvals) if topvals else None;coverage=min(.75,len(players)/22.0*.75);explicit=int(rec["explicit"] or 0)
            expected_ready+=int(expected is not None);explicit_starter_teams+=int(explicit>=7)
            meta={"source":"espn-prematch-roster","roster_players":len(players),"explicit_starters":explicit,"events":rec["events"],"historical_continuity_available":False,"paths":sorted(rec["paths"])[:6]}
        ...
        result={"status":"success" if written>0 else "empty","prematch_events":len(rows),"events_with_team_groups":events_with_groups,"groups_seen":groups_seen,
                "teams_written":written,"expected_xi_ready":expected_ready,"explicit_starter_teams":explicit_starter_teams,"schema_signature":shape}
        print("ESPN_PREMATCH_PLAYER_BRIDGE_RESULT",json.dumps(result,separators=(",",":")),flush=True);return result

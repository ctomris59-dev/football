...
LOOKAHEAD_DAYS=int(os.getenv("BBS_LINEUP_LOOKAHEAD_DAYS","6"))
REQUEST_DELAY=float(os.getenv("BBS_LINEUP_REQUEST_DELAY_SECONDS","0.20"))
...
log=logging.getLogger("bbs-lineups")

SCHEMA="""
CREATE TABLE IF NOT EXISTS bbs_lineup_snapshots(
    espn_event_id TEXT NOT NULL,
    bbs_match_id TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS bbs_lineup_runs(
    id BIGSERIAL PRIMARY KEY,started_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),finished_at TIMESTAMPTZ,
    status TEXT NOT NULL,api_calls INTEGER NOT NULL DEFAULT 0,league_calls INTEGER NOT NULL DEFAULT 0,
    candidate_matches INTEGER NOT NULL DEFAULT 0,mapped_matches INTEGER NOT NULL DEFAULT 0,
    lineup_calls INTEGER NOT NULL DEFAULT 0,lineups_present INTEGER NOT NULL DEFAULT 0,message TEXT
);
"""
ALIASES={"man utd":"manchester united","man united":"manchester united","man city":"manchester city","nottm forest":"nottingham forest","wolves":"wolverhampton wanderers","spurs":"tottenham hotspur","tottenham":"tottenham hotspur","milan":"ac milan","inter":"inter milan","psg":"paris saint germain","paris sg":"paris saint germain"}
def canon(v:Any)->str:
    s=unicodedata.normalize("NFKD",str(v or "")).encode("ascii","ignore").decode().lower().replace("'","")
    s=re.sub(r"\b(fc|cf|ssc|ac|club|football club)\b"," ",s);s=re.sub(r"[^a-z0-9]+"," ",s).strip();s=re.sub(r"\s+"," ",s)
...
class Importer:
    ...
    def match_row(self,dt,home,away,rows):
        ...
        for r in rows:
            ...
            rd=parse_dt(first(r,("start_time","startTime","kickoff","date","match_date")))
            if not rd:continue
    ...
    def run(self)->Dict[str,Any]:
        rid=self.conn.execute("INSERT INTO bbs_lineup_runs(status) VALUES('running') RETURNING id").fetchone()[0]
        league_calls=candidates=mapped=lineup_calls=present=0
        try:
            upcoming=self.upcoming();by_league={}
            for key,name in LEAGUES:
                p=self.get("/v1/matches",{"sport":"football","league":key});league_calls+=1
                rows=rows_from_matches(p);by_league[name]=rows;candidates+=len(rows)
            ...
            for eid,dt,league,home,away in upcoming:
                ...
                # Count only obvious lineup/bench arrays; raw is retained for later exact parsers.
                starters=count_role(raw,("starting","starters","startingxi","starting_xi"))
                bench=count_role(raw,("bench","substitutes","subs"))
                # If source nests by home/away, count side-specific arrays separately where possible.
                data=raw.get("data") if isinstance(raw.get("data"),dict) else raw
                hobj=data.get("home") if isinstance(data,dict) else None;aobj=data.get("away") if isinstance(data,dict) else None
                hs=count_role(hobj,("starting","starters","startingxi","starting_xi")) if hobj is not None else None
                ass=count_role(aobj,("starting","starters","startingxi","starting_xi")) if aobj is not None else None
                hb=count_role(hobj,("bench","substitutes","subs")) if hobj is not None else None
                ab=count_role(aobj,("bench","substitutes","subs")) if aobj is not None else None
                ...
                explicit=find_bool(raw,{"confirmed","isconfirmed","is_confirmed","lineupsconfirmed"})
                has=bool((hs or 0)+(ass or 0)+(hb or 0)+(ab or 0));present+=int(has)
            ...
            self.conn.execute("UPDATE bbs_lineup_runs SET finished_at=NOW(),status='success',api_calls=%s,league_calls=%s,candidate_matches=%s,mapped_matches=%s,lineup_calls=%s,lineups_present=%s,message='ok' WHERE id=%s",(self.calls,league_calls,candidates,mapped,lineup_calls,present,rid))
            res={"status":"success","api_calls":self.calls,"candidate_matches":candidates,"mapped":mapped,"lineup_calls":lineup_calls,"lineups_present":present};log.info("BBS_LINEUPS_RESULT %s",json.dumps(res,separators=(",",":")));return res

#!/usr/bin/env python3
"""Unified current player-availability enrichment v3.

Same semantics as v2, with explicit psycopg Jsonb adaptation for FotMob injury
payloads so Python dict/list JSON cannot break the UPDATE statement.
"""
...
def run_enrich(database_url: Optional[str] = None) -> Dict[str, Any]:
    ...
    with psycopg.connect(db, autocommit=True) as conn:
        ...
        for event_id, hour in rows:
            ...
            lineup_confirmed = bool((bbs_present and bbs_confirmed) or (sofa_present and sofa_confirmed))
            match_specific_present = bool(bbs_present or sofa_present)
            sources = []
            if current_injury_present: sources.append("fotmob_current_injury")
            if bbs_present: sources.append("bbs_match_lineup")
            if sofa_present: sources.append("sofascore_match_specific")
    ...
    result = {
        ...
        "match_specific": match_specific, "confirmed_lineups": confirmed,
    }
    print("AVAILABILITY_ENRICH_V3_RESULT", json.dumps(result, separators=(",", ":")))
    return result
